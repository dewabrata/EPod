...
public class AdapterListBasicDelDoc extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public AdapterListBasicDelDoc(Context context, List<Dataorder> items) {
        this.items = items;
        ctx = context;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof OriginalViewHolder) {
            ...
            view.name.setText(p.getShipmentNo());
            view.description.setText(p.getShipName());
           // Tools.displayImageRound(ctx, view.image, p.image);
            view.image.setImageResource(R.drawable.img_number_verification);
